...
use crate::domain::shopping_item::use_cases::clear_bought::ClearBoughtItemsUseCase;
...
impl ClearBoughtItemsUseCase for ClearBoughtItemsUseCaseImpl {
    async fn execute(&self) -> Result<u64, ShoppingItemError> {
        ...
        let count = self.repository.delete_bought().await?;
...
mod tests {
    ...
    async fn should_delete_only_bought_items() {
        ...
        mock_repo.expect_delete_bought().returning(|| Ok(3));
    ...
    async fn should_return_zero_when_no_bought_items() {
        let mut mock_repo = MockShoppingItemRepo::new();
        mock_repo.expect_delete_bought().returning(|| Ok(0));
